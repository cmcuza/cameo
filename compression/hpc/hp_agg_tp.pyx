...
from compression.lpc cimport tp 
...
cdef Py_ssize_t extract_1st_tps_importance(HPAcfAgg *model, double[:] x, 
    ...
    for i in range(1, n - 1):
        if not tp.is_line(x, i) and (tp.is_convex(x, i) or tp.is_concave(x, i)):
...
cpdef np.ndarray[np.uint8_t, ndim=1] simplify_by_agg_tp(double[:] y, Py_ssize_t nlags, Py_ssize_t kappa, double acf_threshold):
    ...
    while tp_importance_heap.values[0].value < inf:
        ...
        if min_node.left > 0:
            ...
            node_importance = tp.compute_importance(y, tp_importance_heap.values[left_node_index])
        ...
        if min_node.right < tp_importance_heap.m_size-1:
            ...
            node_importance = tp.compute_importance(y, tp_importance_heap.values[right_node_index])
