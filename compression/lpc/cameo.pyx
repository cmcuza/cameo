...
cdef void parallel_look_ahead_reheap(AcfAgg *acf_agg, Heap *acf_errors, unordered_map[int, int] &map_node_to_heap,
    ...
    cdef:
        int i, ii, left_node_index, right_node_index, start, end, \
    ...
    with nogil, parallel(num_threads=num_threads):
        ...
        for i in prange(real_size):
            ...
            if start + 2 < end:
                ...
                # nb_imp[i] = inc_acf.look_ahead_interpolated_impact(acf_agg, y, raw_acf, start, end)
            else:
                ...
                if delta != 0:
                    ...
                    nb_imp[i] /= acf_agg.nlags
                # nb_imp[i] = inc_acf.look_ahead_impact(acf_agg, y, raw_acf, x_a, start + 1)
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef void look_ahead_reheap(AcfAgg *acf_agg, Heap *acf_errors, unordered_map[int, int] &map_node_to_heap, double [:]y, double *raw_acf, const Node &removed_node, int &hops):
    cdef:
        int i, ii, left_node_index, right_node_index, start, end, \
            real_size, n, lag, num_deltas, index, num_lags, max_nd
        double x_a, delta, delta_ss, ys, yss, xs, xss, sxy, lag_acf, slope
...
@cython.boundscheck(False)
@cython.wraparound(False)
cpdef np.ndarray[np.uint8_t, ndim=1] simplify_by_sip(long [:] x, double[:] y, int hops, int nlags, double acf_threshold):
    cdef:
...
@cython.boundscheck(False)
@cython.wraparound(False)
cpdef np.ndarray[np.uint8_t, ndim=1] parallel_simplify_by_sip(long [:] x, double[:] y,
                                                                   int hops, int nlags,
...
@cython.boundscheck(False)
@cython.wraparound(False)
cpdef np.ndarray[np.uint8_t, ndim=1] parallel_simplify_by_blocking_mse(long [:] x, double[:] y, int hops, int nlags, double acf_threshold, int num_threads):
    cdef:
        int start, end, lag, n = x.shape[0]
        double ace = 0.0, x_a, right_area, left_area, c_acf, inf = INFINITY
        double * raw_acf = <double *> malloc(nlags * sizeof(double))
...
@cython.boundscheck(False)
@cython.wraparound(False)
cpdef np.ndarray[np.uint8_t, ndim=1] parallel_simplify_by_blocking_compression_centric(long [:] x, double[:] y, int hops, int nlags, double cr, int num_threads):
    cdef:
