...
cimport cython
from cython_modules cimport inc_acf_agg, inc_acf
...
cdef void parallel_look_ahead_reheap(AcfAgg *acf_model, Heap *acf_errors, unordered_map[int, int] &map_node_to_heap,
    ...
    cdef:
        ...
        double *nb_imp, *sum_agg_deltas, *sxy_s, *ys_s, *yss_s, *xs_s, *xss_s
    ...
    # print('It reached here with max_nd', max_nd)
    ...
    with nogil, parallel(num_threads=num_threads):
        ...
        for i in prange(real_size):
            ...
            if start + 2 < end:
                # print('It reached here with start and end', start, end)
                ...
                # print('It went out here', start, end)
            else:
                x_a = (y[end] - y[start]) / (end - start) + y[start]
                start = start + 1
                # print('It reached here with start', start)
...
@cython.boundscheck(True)
@cython.wraparound(True)
cdef void look_ahead_reheap(AcfAgg *acf_model, Heap *acf_errors, unordered_map[int, int] &map_node_to_heap,
...
@cython.boundscheck(True)
@cython.wraparound(True)
cpdef np.ndarray[np.uint8_t, ndim=1] simplify_single_thread(double[:] y, int hops,
    ...
    math_utils.compute_acf_agg_fall(acf_model, y, aggregates, raw_acf, error_values, n, kappa)
    ...
    while acf_errors.values[0].value < inf:
        ...
        if ace >= acf_threshold:
            print('it reached here with', ace, acf_threshold)
            break

        no_removed_indices[min_node.ts] = False
        heap.update_left_right(acf_errors, map_node_to_heap, min_node.left, min_node.right)
        look_ahead_reheap(acf_model, acf_errors, map_node_to_heap, y, aggregates, raw_acf, min_node, hops, kappa)
...
cpdef np.ndarray[np.uint8_t, ndim=1] parallel_simplify_by_blocking(double[:] y, int hops,
    ...
    inc_acf_agg.fit(acf_model, y, aggregates, kappa)  # extract the aggregates
    inc_acf.get_acf(acf_model, raw_acf)  # get raw acf
    math_utils.compute_acf_agg_fall(acf_model, y, aggregates, raw_acf, error_values, n, kappa) # computing the areas for all triangles
    heap.initialize_from_pointer(acf_errors, map_node_to_heap, error_values, n) # Initialize the heap
    # logs = list()
    ...
    while acf_errors.values[0].value < inf:
        ...
        no_removed_indices[min_node.ts] = False
        heap.update_left_right(acf_errors, map_node_to_heap, min_node.left, min_node.right)
        parallel_look_ahead_reheap(acf_model, acf_errors, map_node_to_heap, y, aggregates, raw_acf, min_node, hops, kappa, num_threads)
