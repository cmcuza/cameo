...
cdef void interpolate_update_outside_lags(AcfPtr model, double[:] x, const int &start, int &end):
    ...
    cdef int i, index, lag, num_lags, num_deltas
    cdef double *deltas
    cdef double *x_as
...
cdef void interpolate_update_inside_lags(AcfPtr model, double[:] x, const int &start, int &end):
    cdef double delta, delta_ss, slope
    cdef int i, index, lag, num_lags, num_deltas, n
    cdef double *deltas
    cdef double *x_as
    num_deltas = (end - start - 1)
    slope = (x[end] - x[start]) / (end - start)
...
cdef double look_ahead_interpolated_impact(AcfPtr model, double[:] x, double *raw_acf, const int &start, int &end) nogil:
    ...
    cdef int i, index, lag, num_deltas, n # num_lags
    cdef double *deltas, *sxy, *ys, *yss, *xs, *xss
