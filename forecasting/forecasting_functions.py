import gc
import torch
from forecasting.models.lstm import LSTMModel
...
def get_lstm_forecasting(data_name, file_path, file_name):
    BASE_DIR = '.'
    reader = TSFReader()
    full_file_path_and_name = os.path.join(file_path, file_name)
    # Assuming file_path and BASE_DIR are defined
    split_string = file_path.split("/")[2:]
    ...
    file_path_ = os.path.join(results_folder_forecasts, f"{data_name}_lstm.txt")
    if os.path.exists(file_path_):
        os.remove(file_path_)
    ...
    for index, row in loaded_data.iterrows():
        ts = np.asarray(row.series_value)[:-fh].copy()
        model = LSTMModel()
        model.fit(ts, lookback)
        forecast = model.predict_future(ts, lookback)
        if data_name == "pedestrian":
        ...
        forecast_str = f'T{index},'+','.join([str(p) for p in forecast.squeeze()])+'\n'
        forecasting_results_file_.write(forecast_str)
    ...
    r_code = f"""
        BASE_DIR = "."
        source(file.path(BASE_DIR, "forecasting", "fixed_horizon_functions.R", fsep = "/"))
        only_forecasting_metrics("{data_name}", "{file_path}", "lstm", "{file_name}")
    """

    robjects.r(r_code)


def get_tcn_forecasting(data_name, file_path, file_name):
    BASE_DIR = '.'
    reader = TSFReader()
    full_file_path_and_name = os.path.join(file_path, file_name)
    # Assuming file_path and BASE_DIR are defined
    ...
    for subfolder in split_string:
        ...
        results_folder_error = os.path.join(results_folder_error, subfolder)
        results_folder_forecasts = os.path.join(results_folder_forecasts, subfolder)
        # results_folder_model_hyper = os.path.join(results_folder_model_hyper, subfolder)
...
def process_data_item_forecasting(data_item):
    ...
    if forecasting_model == 'arima':
        ...
        get_tcn_forecasting(data_name, './data/raw', file_name)
    else:
        raise ValueError(f'Forecasting model {forecasting_model} is not defined')
