...
source(file.path(BASE_DIR, "forecasting", "models", "local_univariate_models.R", fsep = "/"))
...
SEASONALITY_VALS[[3]] <- c(144, 1008)
SEASONALITY_VALS[[4]] <- c(96, 672)
SEASONALITY_VALS[[5]] <- c(48, 336)
SEASONALITY_VALS[[6]] <- c(24, 168)
...
get_ar_order <- function(file_path_name, index) {
  data <- read.csv(file_path_name, header = FALSE)  # Replace 'your_file.txt' with your actual file path
  row <- data[index, -1]  # We use -1 to exclude the first column
  return(as.numeric(row))
}


only_forecasting_metrics <- function(dataset_name, file_path, method, input_file_name, key = "series_name", index = "start_timestamp"){
  split_string <- strsplit(file_path, "/")[[1]]
  split_string <- split_string[3:length(split_string)]

  results_folder_exec <- file.path(BASE_DIR, "results", "fixed_horizon_execution_times", fsep = "/")
  results_folder_error <- file.path(BASE_DIR, "results", "fixed_horizon_errors", fsep = "/")
  results_folder_forecasts <- file.path(BASE_DIR, "results", "fixed_horizon_forecasts", fsep = "/")
  results_folder_model_hyper <- file.path(BASE_DIR, "results", "fixed_horizon_model_hyper", 'raw', fsep = "/")

  for (subfolder in split_string){
    if (!is.na(subfolder)){
        results_folder_exec <- file.path(results_folder_exec, subfolder, fsep = "/")
        results_folder_error <- file.path(results_folder_error, subfolder, fsep = "/")
        results_folder_forecasts <- file.path(results_folder_forecasts, subfolder, fsep = "/")
        # results_folder_model_hyper <- file.path(results_folder_model_hyper, subfolder, fsep = "/")
    }
  }

  # Loading data from the .tsf file
  loaded_data <- convert_tsf_to_tsibble(file.path(BASE_DIR, file_path, input_file_name, fsep = "/"), VALUE_COL_NAME, key, index)
  ...
  for(s in seq_along(all_serie_names)){
    ...
    if (nrow(series_data) < max(seasonality)){
      seasonality <- seasonality[1:length(seasonality)-1]
    }
  ...
  forecast_matrix <- read.csv(file.path(results_folder_forecasts, paste0(dataset_name, "_", method, ".txt"), fsep = "/"), header = F)
  forecast_matrix <- as.matrix(forecast_matrix[-1])
  calculate_errors(forecast_matrix, actual_matrix, train_series_list, seasonality, file.path(results_folder_error, paste0(dataset_name, "_", method), fsep = "/"))

}

do_fixed_horizon_local_forecasting <- function(dataset_name, file_path, method, input_file_name, key = "series_name", index = "start_timestamp", load_raw_model_hyper = FALSE, external_forecast_horizon = NULL, integer_conversion = FALSE){
  ...
  dir.create(results_folder_exec, showWarnings = FALSE, recursive=TRUE)
  dir.create(results_folder_error, showWarnings = FALSE, recursive=TRUE)
  dir.create(results_folder_forecasts, showWarnings = FALSE, recursive=TRUE)
  dir.create(results_folder_model_hyper, showWarnings = FALSE, recursive=TRUE)
  
  # dir.create(file.path(BASE_DIR, "results", "fixed_horizon_forecasts", fsep = "/"), showWarnings = FALSE, recursive=TRUE)
  if (!load_raw_model_hyper)
    file.remove(file.path(results_folder_model_hyper, paste0(dataset_name, "_", method, ".txt")))

  file.remove(file.path(results_folder_forecasts, paste0(dataset_name, "_", method, ".txt")))
  ...
  for(s in seq_along(all_serie_names)){
    ...
    if(!is.null(index))
      start_date <- start(as.ts(train_series_data[0:100, c(index, VALUE_COL_NAME)], frequency = max(seasonality)))
    ...
    # Forecasting
    current_method_forecasts = NA
    if (!load_raw_model_hyper){
        current_method_forecasts <- eval(parse(text = paste0("get_", method, "_forecasts(series, forecast_horizon)")))
    }
    else{
        ar_order <- get_ar_order(file.path(results_folder_model_hyper, paste0(dataset_name, "_", method, ".txt")), s)
        current_method_forecasts <- eval(parse(text = paste0("get_", method, "_forecasts(series, forecast_horizon, ar_order)")))
    }
    model_hyper = NA
    if(typeof(current_method_forecasts) == "list"){
      model_hyper =  current_method_forecasts[[2]]
    ...
    write.table(t(c(all_serie_names[s], model_hyper)), file.path(results_folder_model_hyper, paste0(dataset_name, "_", method, ".txt"), fsep = "/"), row.names = FALSE, col.names = FALSE, sep = ",", quote = FALSE, append = TRUE)
    write.table(t(c(all_serie_names[s], current_method_forecasts)), file.path(results_folder_forecasts, paste0(dataset_name, "_", method, ".txt"), fsep = "/"), row.names = FALSE, col.names = FALSE, sep = ",", quote = FALSE, append = TRUE)
  }
  ...
  write(paste(exec_time, attr(exec_time, "units")), file = file.path(results_folder_exec, paste0(dataset_name, "_", method, ".txt"), fsep = "/"), append = FALSE)
