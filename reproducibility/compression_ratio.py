...
def run_cr_experiments():
    factory = DataFactory()
    datasets = list(factory.loaders.keys())[:-1]
    cr_nrmse_results = {"dataset": [], "line_simplification": [], "error_bound": [], "cr": [], "nrmse": []}

    try:
        previous_results = pd.read_csv(os.path.join('reproducibility', 'results', 'cr_nrmse_results.csv'))
    ...
    for dataset in datasets:
        if dataset != 'uk_electrical_demand':
            continue
        ...
        for error_bound in error_bounds:
            for target in ['sip', 'pip', 'vw', 'tp']:
                ...
                cr_nrmse_results["nrmse"].append(np.round(nrmse(decomp_y, y), 4))
                cr_nrmse_results["cr"].append(round(len(y) / len(comp_y), 2))

            pd.DataFrame(cr_nrmse_results).to_csv(os.path.join('reproducibility', 'results', 'cr_nrmse_results.csv'),
                                                  index=False)
